...
# Get ssn
myload(ssn, dir = mypath("data-raw", "naiades_ssn", "temperature"))
...
MoreArgs = list(.data = yearly_avg_temp,
  ssn_dir = mypath("data-raw", "naiades_ssn", "temperature"))
)
...
data_dir <- mypath("data-raw", "naiades_temperatures")
options(mc.cores = 30)
sapply(basin, function (basin) {
  ...
  obj_path <- mypath("data-raw", "naiades_temperatures", paste0(obj_name,".rda"))
