...
prepare_basin_data(basin = my_hydro_basin, group_var = basin_name, streams = rht,
  ...
  crs = 2154, save_path = mypath("data-raw", "fire_ssn", "temperature"))

#options(mc.cores = 2)
sapply(my_hydro_basin$basin_name, function (basin) {
  # Prepare path:
  mnt_path <- mypath("data-raw", "fire_ssn", "temperature", basin, "dem.tif") 
  pred_path <- paste0(mypath("data-raw", "fire_ssn", "temperature", basin),
    "/", basin, "_pred_sites.shp")
  # load data:
  load(paste0(mypath("data-raw", "fire_ssn", "temperature", basin), "/", basin,
      "_obs.rda"))
  load(paste0(mypath("data-raw", "fire_ssn", "temperature", basin), "/", basin,
    "_streams.rda"))
  ...
  prepare_ssn(grass_path = "/usr/lib/grass76/",
    ...
    ssn_path = mypath("data-raw", "fire_ssn", "temperature", paste0(basin, ".ssn")),
    slope = FALSE)
  })
