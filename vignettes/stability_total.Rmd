...
kable(car::vif(cv_sp_mod))
```

```{r}
log_rich_mod <- lm(log_rich ~ temperature + DBO + fungicides + herbicides + insecticides, dsem)
kable(car::vif(log_rich_mod))
```

```{r}
connect_mod <- lm(connectance_corrected_med ~ temperature + DBO + fungicides + herbicides + insecticides, dsem)
kable(car::vif(connect_mod))
```

```{r}
dbo_mod <- lm(DBO ~ nitrates + phosphore + fungicides + herbicides + insecticides, dsem)
kable(car::vif(dbo_mod))
```

### Check individual linear models
...
test %<>%
  mutate(
    diagnos = map2(model, resp, function (mod, titles){
      p <- recordPlot()
      par(mfrow = c(1, 2))
      plot(mod, which=c(2,1))
      title(as.character(titles), line = -2, outer = TRUE)
      p
  }
      ),
  )
