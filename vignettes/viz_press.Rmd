...
myload(yearly_press_interp_mv_avg, dir = data_common)
...
```{r}
myload(temporal_evt_press, dir = mypath("data"))
press <- temporal_evt_press %>%
  gather(category, press, -id)
```
...
```{r}
kable(cor(select(temporal_evt_press, -id), method = "spearman"))
```
...
stab_cat_press <- synchrony %>%
  left_join(rename(press, station = id))

formula <- y ~ x #needed for stat_poly_eq
ggplot(stab_cat_press, aes(x = press, y = cv_com)) +
  geom_point() + 
  ...
  stat_poly_eq(
    ...
    parse = TRUE) +
  labs(x = "Pressure", y = "CV of the community")
```
```{r}
