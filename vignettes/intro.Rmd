...
- [ ] Re-run willem analysis
  - [ ] Function structure:
    ...
    - [ ] network methods:
      - [ ] connectance, average trophic level, richness
    - [ ] network analysis:
      - [ ] variance of the biomass series 
- [ ] Assess quality of the dataset:
  - [x] Which community are best followed in time?
  - [x] Where are they ?
  - [ ] When new data, cf Edeline filtering methods
  - [ ] Algorithm for op fusion (when poor fisheries):
    `../data-raw/generate_analysis_dataset.R`
  - [x] When selecting station, update all `_analysis` dataset (env).
    - [x] expect for `fish_length` bc we need all the dataset for metaweb (I
      guess)
    - [ ] select only *complete* station
