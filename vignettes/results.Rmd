...
mutate(x_pos = ymd_hms("2015-01-01T15:00:00"), y_pos = 5000,
...
```{r}
myload(station_analysis, dir = data_common)
myload(region_polygon, dir = data_common)
...
ggplot(data = region_polygon) +
    geom_sf() +
    geom_sf(data = station_stab, aes(color = biomass_stab), size = 4) +
...
betadiv <- temporal_community_metrics %>%
  select_at(vars(matches("station|betadiv")))
stab_com_net <- left_join(avg_network, betadiv) %>%
  gather(metrics, values, colnames(select_at(., vars((matches("med")))))) %>%
  gather(betadiv, betadiv_val, colnames(select_at(., vars((matches("betadiv")))))) %>%
  mutate(metrics = str_replace(metrics, "_med", "")) %>%
  filter(metrics != "nbnode")
```
