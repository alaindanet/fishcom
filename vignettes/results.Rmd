...
biomass_com <- temporal_community_metrics %>%
  select(richness_avg, biomass_cv, richness_cv) %>%
  gather(type, richness, richness_avg, richness_cv)
formula <- y ~ x #needed for stat_poly_eq
qplot(richness, biomass_cv, data =  biomass_com, geom = "point") +
  facet_grid(cols = vars(type), labeller = label, scales = "free_x") +
  ...
  stat_poly_eq(
    ...
    parse = TRUE) +
  xylabs(x = "richness", y = "biomass_cv")
```
...
p <- ggplot(cv_biomass_net_avg, aes(y = biomass_cv, x = values)) +
  geom_point() +
  facet_grid(vars(metrics), scales = "free_x") +
  geom_smooth(method = 'lm') +
  stat_poly_eq(
    formula = formula,
    eq.with.lhs = "italic(hat(y))~`=`~",
