...
sampling_point_col <- c(
  "id", "sta_id", "code_sandre", "code_wama",
...
There are 22% of the operation where the station id is na... Why?

```{r}
filter(op, is.na(station))
filter(op, is.na(pop_id))

filter(sampling_point, is.na(id))
# All the sampling point have ids
...
filter(sampling_point, is.na(sta_id) & is.na(code_sandre) & is.na(code_wama)) 
# 22...

## How many op in them:
filter(op, pop_id %in% na_station$id)
### 20 %
filter(sampling_point, is.na(code_sandre))
filter(station_id, is.na(station))
filter(station_id, is.na(pop_id))
```
...
EPSG[grep("RGFG95 / UTM 22", EPSG$note), 1:2]
EPSG[grep("RGR92 / UTM 40", EPSG$note), 1:2]
...
Go to `../run/02_build_fish_lot.R` to generate the fish.
