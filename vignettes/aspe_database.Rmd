...
```{r}
op <- select(op, -env_op_col)
filter(op, espece_ciblee == TRUE) 
#save(op, file = #HERE)
```
```{r}
...
colnames(op_desc)
desc_op_col <-  
aspe_tables <- DBI::dbListObjects(con, prefix = DBI::Id(schema="aspe"))$table %>%
  unlist %>%
  str_replace_all(., "<Id> schema = aspe, table = ", "")
unlist(aspe_tables)
