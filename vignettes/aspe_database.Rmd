...
```{r}
lot_poissons %<>%
  filter(lop_pre_id %in% collect(pre_ele)$pre_id) %>%
  filter(lop_id %in% op$id)
...
filter(lot_poissons, lop_id == 3749287)
filter(ind_measure, mei_lop_id == 3749287)
lot <- ind_measure %>% dplyr::filter(mei_lop_id == 3749287) %>%
  collect() %>%
