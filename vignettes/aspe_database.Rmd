...
op_env2 %<>% mutate(temp_max_moyenne_eau = ifelse(temp_max_moyenne_eau <= 0, NA, temp_max_moyenne_eau))

op_env2 %<>% mutate(surface_calculee = ifelse(surface_calculee <= 0, NA, surface_calculee))
op_env2

HERE
#TODO: complete op_env2 with op_env
# Change colnames of op_env2
...
## How many op in them:
filter(op, pop_id %in% na_station$id) %>% nrow
filter(op, pop_id %in% na_station$id) %>% nrow / nrow(op) * 100
### 20 %
```
