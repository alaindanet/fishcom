...
```{r}
sync_pred <- sync_pair_int2 %>%
  mutate(interact = ifelse(interact, "Trophically linked", "Non trophically
      linked")) %>%
  ggplot(aes(y = sync, x = as.factor(interact))) +
  geom_boxplot() +
  labs(y = "Synchrony", x = " Trophic interactions betweem trophic species pairs", fill = "Cannibalism")
```
...
save_plot(file = mypath(
    ...
    plot = sync_pred,
    base_height = 4.2, base_asp = 1.2#1.618 
    )
```
...
st_wgs <- station_analysis %>%
  filter(id %in% sem_data$station)
...
air_temp <- enframe(air_mean_temp, name = "station", value = "air_95_18") %>%
  ...
  air_95_05 = rowMeans(
      tmean[, which(colnames(tmean) == "01_1995"):which(colnames(tmean) == "12_2005")]
    )
  )
...
pairs(
  temp_avg_comp[, c(2, 4, 5)], pch = 19 
)
dev.off()

mat <- round(cor(temp_avg_comp[, c(2, 4, 5)]), 3)
write.table(mat,
