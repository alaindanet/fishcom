...
belong to the same species based on fecundation criteria.
...
in streams because it had the follow the path of the streams. Furthermore,
...
# Species list and food diet data

```{r sp-list}
myload(species, fish_diet_shift, dir = mypath("data"))
kable(species,
  format = "latex",
  booktabs = T,
...
```{r food}
kable(select(fish_diet_shift, - species_name, -remark),
  format = "latex",
  booktabs = T,
  label = paste0("onto-diet"),
  caption = "Ontogenic diet shift for each species"
  ) %>%
kable_styling(latex_options =c("striped", "scale_down"))
```
...
intercept_sem <- map_dbl(mod_stab_list[!mask_drop_lm],
  get_intercept) %>%
...
```{r sensitivity, message = FALSE, fig.dim = c(10, 7), fig.cap = "Sensivity analysis with different dataset", cache = FALSE}
library(furrr)
...
p_sens_bm <- plot_sensivity_analysis(.data = sem_bm_results) +
...
p_sens_stab %<>% + theme(legend.position = "none")
p <- plot_grid(
