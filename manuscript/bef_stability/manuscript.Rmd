...
ctrl_lmer <- lmerControl(optCtrl = list(method = 'optim'))
stab_div_mod <- lmer(data = sem_data,
  #control = ctrl_lmer,
...
#
bm_div_mod <- lme(data = sem_data,
  fixed = log_bm ~ log_rich,
...
stab_troph_mod <- lme(data = na.omit(stab_div_troph),
  fixed = log_stab ~ log_rich + troph_group + log_rich:troph_group,
  random = ~ 1 | basin,
  control = ctrl
)
...
filter(stab_div_troph, log_rich < 0)
# Plot left  
p_stab_div <- sem_data %>%
