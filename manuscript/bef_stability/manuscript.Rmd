...
higher connectance can in some cases reverse the positive effects of species
...
[@thebault_trophic_2005]. In addition, recent models revealed new mechanisms by
which foodweb structure affects total ecosystem biomass and its temporal
variability. In particular, the number of trophic levels in foodwebs, which is
associated with the presence of species at higher trophic levels, increases
total primary production [@wang_biodiversity_2018]. The addition of predators has
also cascading effects on both species variability and synchrony at lower
trophic levels [@teng_dynamics_2004; @shanafelt_stability_2018].
Overall, these theoretical studies show that the effects of species richness and
network structure, such as connectance and average trophic level, are
potentially conflicting or synergistic in such a way that they need to be
addressed jointly.
...
species richness. Accounting for environment also allows for an overall
...
[@jennings_weak_2001; @gravel_inferring_2013; @brose_predator_2019]. Especially
in stream communities, including fishes, @brose_predator_2019 found that the
prediction of predator-prey trophic interactions based on body size ratio was
far better in stream ecosystems than in other aquatic and terrestrial
ecosystems. Lastly, most of the diversity-biomass and diversity-stability
...
whether species richness, connectance and average trophic level increase,
decrease or are unrelated to total biomass and its temporal stability. We
further question whether the species richness, connectance, and average trophic
level have antagonistic or mutual effect on total biomass and its temporal
stability. Finally, we question if ecosystem size and average temperature
increase total biomass and its temporal stability.
...
large streams (detailed in appendix B, part 1.1). For streams
...
season spanned from late spring to autumn. One sampling event corresponds to a
sampling a given year in a given sampling station. During sampling, all the
fishes caught were identified. The fishes were released at the end of
the sampling. Additionally, the width and the depth of the stream were 
...
in total biomass were selected (detailed in appendix B, part 2). However, the
results were robust to the relaxation of this hypothesis (Figure S7). These
selection steps resulted in 99 stations distributed over seven hydrographic
basins (Figure \@ref(fig:matmet) A).
...
numerous (Figure S1, appendix B, part 1.1). The body size of the unmeasured
individuals was inferred under the assumption that species body size followed a
normal distribution whose mean and standard deviation were quantified from the
recorded individuals of the sampling (Figure S1, appendix B, part 1.2). The body
mass of each individual was then estimated following the results of two
meta-analyses on fishes [@lleonart_removing_2000; @froese_cube_2006], using an
allometric law $B_i = 0.01 \times l_i^{3.03}$, with $B$ the body mass in grams,
$l$ the body size in millimeters, $i$ the individual and where the coefficients
$0.01$ and $3.03$.  Species biomass and total biomass at each sampling event
were then calculated by summing individual body masses. To account for the
variation in sampling effort among sites, species biomass and total biomass were
corrected by the surface sampled, and expressed in grams by square meter ($gm^{-2}$). In the analysis,
the temporal average of the annual total biomass of a station has been defined
as the median of the total biomass across sampling years.
...
all the sampling events in a given station. As for biomass, we controlled
species richness for sampling effort by dividing the total number of species by
the total surface sampled in the given station. Species richness was then
expressed in number of species per square meter sampled.
...
site and sampling event (Figure \@ref(fig:matmet) C and D).
...
a trophic species (Figure S2). Additionally to the trophic species, we added
...
(grey nodes in Figure \@ref(fig:matmet) B, C & D). The trophic species and the
...
predation window (Figure S2). The fish-resource trophic links were set between
...
the eigenvalues distribution (Figure S4). The two first principal components
...
structure directly affect total biomass (Figure
\@ref(fig:sem-plot) A)
...
transformation in Figure S5 and S6, Table S5). We set the seven
...
trophic level ($R^2 =
.11$, Figure \@ref(fig:sem-plot) B and D). We found that species richness had a
significant negative effect on mean trophic level ($r_\delta = 
`r rich_on_tlvl`$) and connectance ($r_\delta = `r rich_on_ct`$, Figure
\@ref(fig:sem-plot) B and D), meaning that a part of the effects of species richness went 
...
$r_\delta = `r tlvl_on_bm`$ resp., Figure \@ref(fig:sem-plot) B, Table
\@ref(tab:indir-sem)), while connectance had no direct effect on total biomass
(Figure \@ref(fig:sem-plot) B).
...
Figure \@ref(fig:sem-plot) D) and thereby had a total positive
...
(Figure \@ref(fig:sem-plot) D, Table \@ref(tab:indir-sem)).
...
sizes, or when (ii) species have different
...
Contrasting with previous studies on experimental plant communities
[@grace_integrative_2016], aquatic mesocosms [@yvondurocher_warming_2011;
@dossena_warming_2012] and marine fish communities
[@maureaud_biodiversityecosystem_2019], we found that environmental drivers
overall affect total biomass indirectly rather than directly
[@woods_testing_2020]. First, consistent with research showing that foodwebs in
wider streams support more species and biomass [@allan_stream_2007], we found
that the PCA axis related to stream size had a positive effect on total biomass
through a positive effect on mean trophic level and species richness. Greater
ecosystem size might provide a greater diversity of resources through habitat
heterogeneity, and so higher species richness and total biomass by surface unit,
through resource complementarity. Greater diversity of resources can also
maintain more biomass at high trophic level [@post_ecosystem_2000;
@doi_resource_2009]. Second, the positive effect of the PCA axis related to
temperature on total biomass was of the magnitude of the effects of network
structure and species richness, in line with the mounting body of work reporting
important effects of temperature on the structure and biomass of aquatic systems
[e.g.  @dossena_warming_2012; @edeline_ecological_2013;
@emmrich_geographical_2014; @hattab_forecasting_2016; @woods_testing_2020]. We
found that streams with higher average temperature supported higher diversity as
well as higher average trophic level and total biomass, which is in agreement
with recent findings on streams where the positive effects of temperature were
related to larger nutrient supply [@ogorman_unexpected_2017].
...
clear negative effect on the temporal stability through a direct destabilizing
...
with temperature in our study. It has been suggested that enrichment can increase CV by
increasing species biomass fluctuations while higher temperature might decrease
decrease stability through starvation resulting from high metabolic activity
[@tabi_warming_2019].
