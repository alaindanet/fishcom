...
Both the relationship between diversity and ecosystem functioning, and that between food
web complexity and stability, have received considerable attention during the
last decades in ecology. The former relationship remains rarely investigated
...
We are lacking empirical studies that assess jointly how food web structure, in
conjunction with species diversity, affect total biomass production and other
...
Many studies showed that species richness increases temporal stability of
...
butterfly communities [@olivier_urbanization_2020]. However, this relationship has
been mostly explored in single trophic level systems or without accounting for
food web structure, even more so than the relationship between diversity and biomass
production. Several theoretical studies at considering single trophic level have clarified which mechanisms
...
stability [@tilman_biodiversity_2006; @olivier_urbanization_2020].Though most often positive, the relationship between species diversity and asynchrony has also been evidenced to be neutral or negative in natural settings
...
@stouffer_compartmentalization_2011]. In particular, predation has cascading
effects on both species variability and synchrony [@teng_dynamics_2004; @shanafelt_stability_2018] and it affects as
well synchrony between species. In spite a long-standing interest for the links between food web complexity and stability in ecology, to
...
different relationships might also arise from effects of the environment on both
...
Environmental effects might also complicate (or obscure) the relationship
between diversity and structure in driving stability and biomass in natural
systems [@loreau_biodiversity_2001; @he_global_2013;
...
Because their trophic interactions can be inferred, stream fish communities
...
in other aquatic and terrestrial ecosystems. Lastly, most of the diversity-biomass and diversity-stability
...
based on body size and diet. We question whether species richness,
...
station has been defined as the median of the total biomass acrossall times.
...
decomposed the $CV$ of total biomass ($CV_{com}$) into a synchrony ($\phi$) and
a
...
Species richness was computed as the total number of species present across all times
...
feed on plankton while adults may feed on algae, fishes, with bigger individuals
eating bigger
...
by both its species identity and its body size. Then, species identity allowed to
...
The networks metrics were also based on the trophic species,
i.e. body size classes. Connectance describes the level of generalism in the
network and is calculated as the actual number of
...
B) and that species richness affected food web structure
...
species richness and network structure on both total biomass and its temporal
stability. We found that species richness and mean
...
extends the evidence that species richness decrease synchrony to the case of empirical
...
degree of omnivory in food webs, which combined with strong trophic interactions
...
such an absence of an effect might be explained by the fact that our data lack information about the
...
lower trophic levels as predicted by food chain models in the presence of
strong top-down control [@barbier_pyramids_2019; @shanafelt_stability_2018] but it
is not sure that it holds when considering omnivorous species
[@wang_biodiversity_2016]. An alternative explanation is that higher mean
trophic level leads to stronger top-down control and resource complementarity
...
stability than synchrony [@tilman_biodiversity_2006; @bluthgen_land_2016;
...
communities, which can raise synchrony between prey species which share the
same predator [@raimondo_interspecific_2004] and thus decreases the
occurrence of compensatory population dynamics. Alternatively, it is possible that, in
aquatic foodwebs, the occurrence of large oscillatory dynamics
...
stability, empirical ecology has focused on species richness but often did not considere network
structure. Furthermore, both fields have largely used different stability
metrics. Our work is hence contributing to bridging the gap between
