...
#total biomass and biomass stability.  
---

Species richness and network structure are at the basis of the predictions of
total biomass, productivity and stability. However, empirical data linking
network structure and ecosystem properties are lacking. We used a dataset of
standardized fish sampling in French streams running from 1995
to 2018. We inferred trophic interactions thanks to recorded body size and diet
literature. We investigated the direct and indirect effects of stream size
and temperature, species richness and network structure on total biomass and its
temporal stability. Species richness and average trophic level had
a positive relationship with total biomass while they had respectively a
negative and positive effect on temporal stability. We found a strong link
between species richness and network structure, highlighting that their effects
should be assessed conjointly. Overall, our study opens fruitful perspectives on
the link between species richness, network structure and ecosystem properties.
...
positive effect of richness on biomass production and stability through time
...
links, for piscivorous and non piscivorous nodes, were set between the
trophic species and the resources nodes present in their life stage.
Lastly, the trophic links between resource nodes were set according to the food
diet data [@allan_stream_2007; @hart_handbook_2008].
...
webs, namely the connectance and mean trophic level. Connectance describes 
...
links and of trophic species plus resource nodes). 
The mean trophic level $\overline{T}$ was computed as the mean trophic level
...
\sum_i^N{B_i}^{-1}}$, with $T_i$ and $B_i$ being respectively the trophic level
and the biomass of the node $i$. The trophic level of the nodes was computed as
one added to the average trophic level of its food item.
...
\@ref(tab:indir-sem)).
The PCA2 had a total negative effect on temporal stability 
...
```{r indir-stab, cache = TRUE, results = TRUE}
caption_sem <- paste0(
...
We found that species richness and mean
...
richness on total biomass might be due to resource use complementarity [@carey_determining_2011], which is
held by niche differentiation [@loreau_biodiversity_1998;
@loreau_partitioning_2001] or to the higher probability to get high biomass
...
thereby increasing stability. However, no relation has been found in
natural forests [@declerck_species_2006]. Our study thus
...
increase in species richness leads to a decrease of population sizes, which in
turn raises the role of demographic stochasticity on population dynamics
...
in butterfly and bat communities. However, the ecological mechanisms leading to
such relationships in foodwebs are not known and this question deserves further
research.
...
biomass is located at higher trophic level, the higher the total biomass is. 
Experimental and theoretical work suggested that a
...
the fish compartment. A reason for such an absence of an effect might be due our
data lacks informations about the lower compartments of the
community, i.e, the primary producers, zooplankton and zoobenthos.
...
omnivorous species [@wang_biodiversity_2016] and if other mechanisms can
generate the same pattern [@thibaut_understanding_2013]. An alternative
explanation is that higher mean trophic level leads to stronger top-down control
and resource complementarity and thus reduce biomass fluctuations in lower
trophic levels [@rooney_structural_2006; @neutel_reconciling_2007].
...
and metrics [@arnoldi_resilience_2016]. However, our measure of connectance
suffers limitation and further research should examine
the effect of connectance on temporal stability in empirical foodwebs. 

## Environment had an high effect on total biomass and its temporal stability 

Contrasting with a previous study in experimental plant communities
[@grace_integrative_2016] and in fish communities [@yvondurocher_warming_2011;
@dossena_warming_2012], we found that environmental drivers overall affect total
biomass indirectly rather than directly. In line with previous studies, we found
that the two PCA axis, respectively related to stream size and temperature, 
had indirect positive effects on total biomass, through a positive direct effect
on species richness [@allan_stream_2007; @ogorman_chapter_2012;
...
mechanisms leading to such remains to be elucidated. One limitation of our study
is that the use of inferred networks and the lack of information about the
foodwebs outside fishes. Our findings and interpretation could be confirmed by
experimental and theoretical works considering foodwebs instead of pure
foodchains. It would be promising to explore the effects of omnivory and
ontogeny, i.e. relaxing the foodchain assumption, on the link between network
...
metrics. Therefore, our work aims to contribute at bridging this gap between
theoretical and empirical ecology. Finally, we found that environmental gradient
related to temperature and ecosystem size have antagonistic effects on biomass
temporal stability, positive through mean trophic level and negative through
species richness. Further research is needed to find the mechanisms behind those
pathways, that are crucial in context of global changes. 
