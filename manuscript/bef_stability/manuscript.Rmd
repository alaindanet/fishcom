...
theoretical studies with very little empirical testing. To fill this gap, we linked
species richness, foodweb structure, annual biomass and its temporal
stability using a standardized monitoring of fish communities in French streams
running from 1995 to 2018. We inferred trophic interactions from
individual body size and diet literature. We reveal that species richness and average
...
on the link among species richness, network structure, community biomass and its
temporal stability.
...
Species richness has been shown to promote higher biomass production and  
higher temporal stability of total biomass in a variety of ecosystems
and taxa. 
Indeed species richness generally increases primary productivity and
...
bacterial consumers [@pennekamp_biodiversity_2018]. 
In parallel, such positive relationship between
...
butterfly communities [@olivier_urbanization_2020].
Despite the diversity of
ecosystems and taxa studied, the relationship between diversity and ecosystem
functioning in experimental and empirical studies remains mostly focused
on single trophic levels, ignoring the effects of foodweb structure. 
This gap is even more striking for the relationship between diversity and
temporal stability of ecosystem functioning, in spite of long-standing interest
for the links between food web complexity and stability in theoretical ecology
[e.g. @may_will_1972; @dunne_network_2006; @neutel_reconciling_2007;
@duffy_why_2009; @stouffer_compartmentalization_2011].

Overall, the evidence from theoretical models suggests that both species
richness and foodweb structure should modulate ecosystem propertiestotal
biomass and their stability of communities. Complementarity in the use of
resources, a main mechanism previously highlighted in single-trophic studies, is
important to understand the effects of species richness in food webs [@thebault_food-web_2003;
@poisot_trophic_2013; @schneider_animal_2016], but in that case it also depends
on food web connectance [@thebault_food-web_2003; @poisot_trophic_2013].
...
of the stabilizing effects of species richness on total biomass in food webs
...
which food web structure affects total ecosystem biomass and its temporal
variability. In particular, the number of trophic levels in food webs, which is
...
The gap in studies that consider jointly effects of diversity and food web
...
between diversity, food web structure and the stability of total community
...
and its temporal stability [@de_boeck_patterns_2017; @hansen_climate_2013;
@bluthgen_land_2016; @oliver_heterogeneous_2010; @franssen_annual_2011;
...
distribution [@yvondurocher_warming_2011; @schwarz_warming_2017]. Environment also
directly affect species richness and food web structure. For instance, larger
ecosystems host food webs with higher diversity and trophic levels
[@mchugh_dual_2010] while temperature can affect food web diversity and
structure in contrasted ways depending on the ecosystem studied
[@ogorman_unexpected_2017; @gauzens_biodiversity_2020]. When analyzing data
from natural communities, the relationships among species richness, total
...
matmet_cap <- paste0( 
  ...
  round(qt_station$bm_std_stab[2],
    2), ". Point size are proportional to the biomass of the trophic species.")

```

```{r matmet, fig.cap = matmet_cap, results = "fig.show", out.height="80%"}
library(knitr)
...
The trophic interaction network was inferred from body size and ontogenic diet shift
...
zoobenthos (grey nodes in Figure \@ref(fig:matmet) B, C & D). The fish trophic
...
database was filled according to fishbase and literature (Table S2). Each
...
biomass (Figure \@ref(fig:sem-plot) A). We assumed that both species richness
and network structure directly affect total biomass (Figure \@ref(fig:sem-plot)
A) and that species richness affected foodweb structure
...
transformation in supplementary informations, Figure S5 and S6, Table S5). We set the seven
...
sem_caption <- paste0(
  ...
  (B) the determinants of the median of annual total biomass and (D) its temporal stability. Red and green
  ...
  second axis of the PCA performed on environmental variables (cf methods)."
)
```
...
.18$, Figure \@ref(fig:sem-plot) B and D). We found that species richness had a
...
direct positive effect on mean trophic level ($r_\delta = `r
env_on_stab$via_tlvl["log_RC2"]`$, Table \@ref(tab:indir-sem)). However, PCA2
...
size, but effects were weaker than for PCA2.
...
Overall, this study is unique by its level of integration, from environmental
...
We found that species richness increased total biomass as already found in salt marshes,
...
[@woods_testing_2020]. The positive effect of species
...
[@loreau_biodiversity_2001]. Our result is also in agreement with a recent
study predicting a positive relationship between animal diversity and total
consumer biomass in size-structured food web models [@schneider_animal_2016].
...
@houlalan__2018], where positive relationships were found between temporal
...
richness leads to a decrease of population sizes which increases the impacts of
...
@gross__2014], a previous meta-analysis suggested that species richness had
often a positive effect on population stability in multi-trophic communities
[@jiang_different_2009].
In foodwebs, the relationship between temporal
...
@brose_allometric_2006 DOWNING à la place de BROSE?], high interaction strength inducing a negative
...
thus might not hold in more complex food webs with omnivorous species, our
...
[@barbier_pyramids_2019; @shanafelt_stability_2018]. An alternative
explanation is that higher mean trophic level leads to a stronger top-down
control and resource complementarity and thus reduce biomass fluctuations in
lower trophic levels [REF?].
...
However, a few studies suggest that greater connectance should decrease
complementarity in resource use between species, which is expected to lead to
...
contradicts numerous previous theoretical studies on the relationship between
...
account the distribution of interaction strengths in the food webs. Indeed, food
web connectance is expected to have different consequences on the temporal
...
Contrasting with previous studies in experimental plant communities
...
the temperature optima of species growth rates and consumption rates and on how
nutrient supply varies with temperature (Uszko et al. 2017). In our case, the
observed increased variability of populations might also relate with factors
covarying with temperature such as the average Biological Organic Demand which
is linked to enrichment.
...
positive effect on total biomass and a negative effect on biomass stability.
Mean trophic level had a positive effect on both total biomass and its temporal
stability. Furthermore, we found that species richness had antagonistic effects
on total biomass, through its effect on mean trophic level. Our study thus
highlights the importance of considering both species richness, network
structure and their dependence, though the mechanisms remain to be elucidated.
Finally, we found that environmental gradient related to temperature and
ecosystem size have antagonistic effects on biomass and its temporal stability,
positive through mean trophic level and negative through species richness.
Further research is needed to find the mechanisms behind those pathways, and
crucial in the context of global change.

One limitation of our study is the use of inferred networks, i.e. not
...
We look forward to seeing our findings and their interpretation tested by future
