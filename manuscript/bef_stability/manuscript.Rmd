...
biomass of the station $i$). Then, temporal stability ($S_i$) was equal to $CV^{-1} =
\mu \sigma^{-1}$.
...
In further analysis, we linearized the mathematical relationship linking
temporal stability to $\overline{CV_{sp, i}}$ and $\phi$ by taking the log of
temporal stability: 

\begin{equation} 
  \begin{aligned}
    S_i &= (\overline{CV_{sp, i}} * \phi_i^\frac{1}{2})^{-1} \\
    \log S_i &= - \log (\overline{CV_{sp, i}} * \phi_i^\frac{1}{2}) \\
    \log S_i &= - \log (\overline{CV_{sp, i}} - \log \phi_i^\frac{1}{2} \\
...
controlled by bottom-up effects will display higher synchrony while  mainly
controlled by the food-webs mainly controlled by top-down effects will display
higher population variability. 
