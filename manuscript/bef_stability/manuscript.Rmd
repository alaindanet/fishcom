...
relationships are central to ecology. However, they remain largely untested for 
foodweb in natural context. Here we study the links among
...
@thebault_relationship_2006; @tilman_biodiversity_2006]. Indeed species richness
...
@hector_plant_1999; @tilman_biodiversity_2014; @grace_integrative_2016], stream
feeders, [@cardinale_species_2002] and aquatic bacterial consumers
...
functioning remains mostly focused on
...
, in spite of long-standing interest for the links
...
studied [@ogorman_unexpected_2017; @gauzens_biodiversity_2020]. In natural
communities, the relationships among species richness, total biomass and its
temporal stability might also arise from effects of the environment on both
diversity and ecosystem functions [@loreau_biodiversity_2001; @he_global_2013;
...
informations [@jennings_weak_2001; @gravel_inferring_2013], especially in stream
communities including fishes [@brose_predator_2019].
...
connectance and average trophic level are related to total biomass and its
temporal stability. We further question how ecosystem size and temperature are
related to total biomass and its temporal stability.
...
sampled alternatively with one of the two sampling protocols, we kept only the
most frequently used. The sampling season spanned from late spring to autumn.
There was low heterogeneity of the sampling month within stations, with the
median sampled month being mid-August while the median of the standard deviation
of the sampled month within station was $0.7$.
...
We described the mechanisms driving the temporal stability of biomass,
...
with $CV_{com, i}$ being the CV of the biomass in the
station $i$, $\overline{CV_{sp, i}}$ the average CV of
...
size class.  Lastly, the trophic links among resource nodes were set according
to the literature [@allan_stream_2007; @hart_handbook_2008].

### Network structure

For each trophic network, corresponding to a particular site and year, we
computed the connectance and mean trophic level. Connectance describes the
level of generalism in the network and is calculated as the actual number of
interactions divided by the number of interactions if all species interact
together ($C = L / N^2$, L and N being resp. the number of links and of nodes).
The trophic level of the nodes was computed as one added to the average trophic
level of its food item. The mean trophic level $\overline{T}$ was 
weighted by the biomass of each trophic species (excluding resource nodes for
which we had no biomass information), so $\overline{T} = \sum_i^N{B_iT_i \times
(\sum_i^N{B_i})^{-1}}$, with $T_i$ and $B_i$ being respectively the trophic
level and the biomass of the node $i$. For each site, the median of connectance
and average trophic level over time was used in subsequent analysis.
...
stream network. We obtained data on water temperature and flow from the naiades
database
...
increasing average water temperature, BOD and decreasing altitude and slope.
...
variables, species richness and foodweb structure [@maureaud_biodiversityecosystem_2019;
@woods_testing_2020]. We assumed that both species richness and network
structure directly affect total biomass (Fig. \@ref(fig:sem-plot)A)
...
synchrony and $\overline{CV_{sp}}$ which in turn determined the temporal
...
were computed by multiplying slopes along the path. Only significant direct
slopes were entered in the calculation of indirect effects. Total effects were
...
axis, related to the stream size, was positively linked to total biomass
...
of PCA1, PCA2 and species richness on total biomass were of the same magnitude.
...
The total effect of PCA2, related to average annual temperature, on temporal
...
species with higher species richness [@loreau_biodiversity_2001]. Our results are 
also in agreement with a model predicting a positive relationship between animal
diversity and total consumer biomass in size-structured foodweb
...
relationship. Therefore, it is possible that our results are due to strong
trophic interactions, leading to more fluctuations in population biomass.
...
thus might not hold in more complex foodwebs with omnivory, our
...
explanation is that higher mean trophic level complexify the biomass pathways
...
[@thebault_trophic_2005]. However, our foodwebs lack information about the lower
compartments of the community, i.e. primary producers, decomposers and the benthic
part of the foodweb. Further research should examine the effects of connectance
on total biomass and its temporal stability in more complete empirical foodwebs. 
...
We found that environmental drivers affect total biomass mostly indirectly
...
While the PCA axis related to stream size had only a weak effect on the
temporal stability of total biomass, the PCA axis related to temperature had a
negative effect on temporal stability through a direct destabilizing
...
temperature [@uszko_effects_2017]. The observed increased
...
its temporal stability to the case of foodwebs in natural settings and
