...
stability and (2) the species synchrony [@loreau_species_2008, @thibaut_understanding_2013].
...
species [@loreau_species_2008, @thibaut_understanding_2013, @bluthgen_land_2016; @olivier_independent_nodate]. However, as far as we know,
...
on stability in mutualistic and trophic networks
[@angelis_stability_1975, @stouffer_, @thebault_stability_2010, @barbier_pyramids_2019, @shanafelt_david_w._stability_nodate, add ref].
...
increase persistence, while nestedness can either increase
[@bascompte, @thebault_stability_2010] or decrease persistence
...
relationships as in many studies [@brose_predator_2019, @poisot_structure_2016, @schneider_body_2012].
...
graph_stab_sem <- build_diag_from_sem(
  stab_sem_rich, 
  p_val_thd = 0.05,
...
export_graph(graph = graph_stab_sem, file_type = p_format,
  ...
  height = p_height - p_height*30/100,
  width = p_width
)
#source(mypath("R", "plot_methods.R")) 
graph_bm_sem <- build_diag_from_sem(
  bm_sem_rich, 
  p_val_thd = 0.05,
  env_x_pos_range = c(from = 0, to = 7.5),
  env_y_pos = 0,
...
export_graph(graph = graph_bm_sem, file_type = p_format,
  ...
  height = p_height - p_height*45/100,
  width = p_width 
)
...
p_stab_div_troph <- stab_div_troph %>%
  ...
  geom_ribbon(data = stab_troph_pred,
    ...
    alpha = .2) +
  annotate("text", x = -2.75, y = .75, label = make_label_rsq(rsq = stab_troph_sum),
    parse = TRUE) +
...
p_bm_div_troph <- stab_div_troph %>%
  ...
  geom_ribbon(data = bm_troph_pred,
    aes(ymin = conf.low, ymax = conf.high, color = NULL, group = troph_group),
    alpha = .2) +
  annotate("text", x = -2.75, y = -1.0, label = make_label_rsq(rsq = bm_troph_sum),
    parse = TRUE) +
  labs(x = richness_lab, y = bm_lab)
