...
relationships are central to ecology. However, they remain largely untested in natural context. Here we study the links among
environmental conditions, richness, food-web structure, annual biomass and its
temporal stability using a standardized monitoring dataset of 99 stream fish communities
...
strength. Second, we show that food-web structure affects community stability,
with a positive effect of mean trophic level, and contrary to
expectation, a destabilizing effect of species richness. Finally, we
...
mediated by changes in population stability rather than synchrony, which contrasts
...
butterfly communities [@houlahan_negative_2017; @olivier_urbanization_2020].
Despite the diversity of ecosystems and taxa studied, the relationship between
diversity and ecosystem functioning remains mostly focused on single trophic
levels, ignoring the effects of food-web structure. This gap is even more
striking for the relationship between diversity and temporal stability , in
spite of long-standing interest for the links between food-web complexity and
stability in theoretical ecology [e.g.  @may_will_1972; @dunne_network_2006;
...
resources is important to understand the effects of species richness in food-webs
[@thebault_food-web_2003; @poisot_trophic_2013; @schneider_animal_2016], but it
also depends on food-webs connectance, i.e. its level of generalism
[@thebault_food-web_2003; @poisot_trophic_2013].
...
higher connectance can counterbalance the positive effects of species
richness on temporal stability of community biomass, by leading to a negative
relationship between species richness and population temporal variability
[@thebault_trophic_2005]. In addition, the presence of species at higher trophic
levels increases total primary production, through stronger top-down effects
associated with higher nutrient uptake [@wang_biodiversity_2018]. The addition of predators has
...
properties, which additionally requires longer-term
perspectives. In this context, recent developments to predict species trophic
interactions [@beckerman_foraging_2006; @gravel_inferring_2013; @portalier_foraging_2019], combined with observational long-term
surveys of ecological communities, can offer the opportunity to investigate the
links between diversity, food-web structure and the stability of total community
...
population abundances, community biomass [@grace_integrative_2016; @woods_testing_2020]
...
food-web structure, as larger ecosystems host food-webs with higher
diversity and trophic levels [@mchugh_dual_2010] whereas temperature can affect
food-web diversity and structure in contrasted ways depending on the ecosystem
studied [@ogorman_unexpected_2017; @gauzens_biodiversity_2020]. In natural
communities, the relationships among species richness, community biomass and its
...
@gravel_inferring_2013; @brose_predator_2019; @portalier_foraging_2019].
...
stations with no temporal trends in community biomass were selected (detailed in
...
decomposing the $CV$ of the community biomass ($CV_{com}$) into a
synchrony ($\phi$) component and a weighted average species $CV$ ($\overline{CV_{sp}}$)
...
with $CV_{com, i}$ being the CV of the community biomass in the
station $i$, $\overline{CV_{sp, i}}$ the average CV at the species-level in the station $i$, weighted by the relative biomass of the species, $B_{k,i}$.
...
($C = L / N^2$, L and N being resp. the number of links and of nodes).
...
the eigenvalues distribution (appendix B, Fig. S4). The two first principal components
explained `r tot_var_expl_pca`% of the variance (appendix B, Fig. S4). A varimax axis rotation was
...
structure directly affect community biomass (Fig. \@ref(fig:sem-plot)A) and that
...
of community biomass (Fig.  \@ref(fig:sem-plot)C). Instead of the
annual community biomass, we modelled the synchrony and $\overline{CV_{sp}}$ which
in turn determined the temporal stability of community biomass. 
...
slope had a $P$ value inferior or equal to $0.05$ were reported in the main text, all the
path values are presented in Table S7 and S8.

Indirect effects on community biomass and its temporal stability
were computed by multiplying slopes along the path. Only significant direct
slopes were included in the calculation of indirect effects. Total effects were
...
trophic level ($R^2 = .11$, Fig. \@ref(fig:sem-plot)B and D). We found a
...
($r_\delta = `r rich_on_ct`$, Fig.  \@ref(fig:sem-plot)B and D), meaning that a
...
of PCA1, PCA2 and species richness on community biomass were of the same magnitude.


## Temporal stability of annual community biomass

By its effects on synchrony and $\overline{CV_{sp}}$, which respectively
measures the level of compensatory dynamics among population and the average
population variability, species richness had a
...
variability ($\overline{CV_{sp}}$) had a stronger negative effect on community biomass
...
species richness on temporal stability of community biomass. The weaker effect of synchrony
might be explained by the presence of generalist predators in stream
communities, which can promote synchrony between prey species that share the
same predator and thus reduces compensatory
population dynamics [@raimondo_interspecific_2004]. Alternatively, it is
possible that, in aquatic food-webs, the occurrence of large oscillatory dynamics
 leads to a higher contribution of population
variability rather than synchrony to community stability
[@mccann_reevaluating_1997]. How both strengths of interactions and the relative
contribution of synchrony and population variability to overall stability vary
between food-webs and single trophic communities is still an open question.
...
temporal stability of community biomass, the PCA axis related to temperature had a
...
increasing population biomass fluctuations while higher temperature might decrease
...
library(knitr)
include_graphics(path = mypath("manuscript", "bef_stability", "figs", "fig1.png"))
```
