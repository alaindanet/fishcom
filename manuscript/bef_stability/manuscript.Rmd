...
com_data <- compute_community_temporal_analysis(.op = op_analysis_bbb, dest_dir = mypath("data", "species"))
# Compute sem dataset 
...
stab_bm_path <- mypath("manuscript", "bef_stability",
...
library(magick)
img_stab_sem <- mypath("manuscript", "bef_stability", "stab_sem.svg") %>%
  image_read_svg(width = 1300, height = 1000)
p_stab_sem_rich <- ggdraw() +
  draw_image(img_stab_sem)
img_bm_sem <- mypath("manuscript", "bef_stability", "bm_sem.svg") %>%
  image_read_svg(width = 1000, height = 700)
...
plot_grid(
  ...
  plot_grid(p_stab_sem_rich, p_bm_sem_rich, labels = LETTERS[2:3]),
  ncol = 1,
  rel_heights = c(.4, .6)
)
```
