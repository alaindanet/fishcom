...
[@grace_integrative_2016] and positive relationships
...
net_method_path <- mypath("manuscript/bef_stability/figs",
  "network_inference.pdf")
net_method <- net_method_path %>%
  magick::image_read()
p_net_method <- cowplot::ggdraw() +
  cowplot::draw_image(net_method)

top <- plot_grid(p_st, p_net_method, ncol = 2, rel_widths = c(.8, 1), labels =
  "AUTO")
p_method <- plot_grid(top, temporal_network, nrow = 2, rel_heights = c(1, .3),
  labels = c("", "C"))
p_method
```
...
to be largely dependant on their body size ratio [cascade model, @cohen_stochastic_1985, @warren_invertebrate_1987, @brose_predator_2019].
...
kable(tmp_stab_indir_table, "latex", booktabs = T, align = "c") 
kable(tmp_bm_indir_table, "latex", booktabs = T, align = "c") 

#kable(rbind(
  #mutate(tmp_stab_indir_table, type = "stab"),
  #mutate(tmp_bm_indir_table, type = "bm")
