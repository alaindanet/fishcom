...
bm_troph_mod <- lmer(data = na.omit(stab_div_troph),
  ...
  formula = log_bm ~ log_rich + troph_group + log_rich:troph_group + (1 | basin)
...
s_co_r <- print_coef(coef_div$stab["log_rich"])
s_co_t <- print_coef(coef_troph$stab["troph_group3"])
s_co_r_t2 <- print_coef(coef_troph$stab["log_rich"])
s_co_r_t3 <- print_coef(coef_troph$stab["log_rich"]+coef_troph$stab["log_rich:troph_group3"])

s_ci_r <- print_ci(stab_div_ci["beta.log_rich",])
s_ci_t <- print_ci(stab_troph_ci["beta.troph_group3",])
s_ci_r_t2 <- print_ci(stab_troph_ci["beta.log_rich",])
s_ci_r_t3 <- print_ci(coef_troph$stab["log_rich"] + stab_troph_ci["beta.log_rich:troph_group3",])
```

```{r print-coef-bm}
# Coefficients
b_co_i <- print_coef(coef_div$bm["(Intercept)"])
b_co_r <- print_coef(coef_div$bm["log_rich"])
b_co_t <- print_coef(coef_troph$bm["troph_group3"])
b_co_r_t2 <- print_coef(coef_troph$bm["log_rich"])
b_co_r_t3 <- print_coef(coef_troph$bm["log_rich"]+coef_troph$bm["log_rich:troph_group3"])

b_ci_r <- print_ci(bm_div_ci["beta.log_rich",])
b_ci_t <- print_ci(bm_troph_ci["beta.troph_group3",])
b_ci_r_t2 <- print_ci(bm_troph_ci["beta.log_rich",])
b_ci_r_t3 <- print_ci(coef_troph$bm["log_rich"] + bm_troph_ci["beta.log_rich:troph_group3",])
```


- Stability:
  - decreases globally with median species richness (MSR)
    - An increase of 1% of MSR decreased biomass stability by
      `r s_co_r`% (`r s_ci_r`)
  - was `r s_co_t`% (`r s_ci_t`) higher for higher trophic group
  - increased by `r s_co_r_t2`% (`r s_ci_r_t2`) and decreased by `r s_co_r_t3`
    (`r s_ci_r_t3`) with 1% increase in MSR for the lower and higher trophic
    group respectively

- Biomass:
  - increases with MSR 
    - An increase of 1% of MSR increased total biomass by
      `r b_co_r` % `r b_ci_r`
  - was `r b_co_t`% `r b_ci_t` higher for higher trophic group
  - increased by `r b_co_r_t2`% `r b_ci_r_t2` and decreased by `r b_co_r_t3`
    (`r b_ci_r_t3`) with 1% increase in MSR for the lower and higher trophic
    group respectively
...
```{r colin-sem, fig.cap = "Variance Inflation factor for each individual model of stability sem"}
#compute_stab_sem_rich(.data = sem_data, get_sem =TRUE)

mod_list <-  
  list(
    nlme::lme(log_rich ~ RC1 + RC2 + RC3 + RC4 + RC5,  random = ~ 1 | basin, data = sem_data),
    nlme::lme(piel ~ RC1 + RC2 + RC3 + RC4 + RC5, random = ~ 1 | basin, data = sem_data),
    nlme::lme(ct ~ RC1 + RC2 + RC3 + RC4 + RC5 + log_rich, random = ~ 1 | basin, data = sem_data),
    nlme::lme(t_lvl ~ RC1 + RC2 + RC3 + RC4 + RC5 + log_rich, random = ~ 1 | basin, data = sem_data),
    #nlme::lme( ~ RC1 + RC2 + RC3 + RC4 + RC5 + log_rich, random = ~ 1 | basin, data = sem_data),
...
names(mod_list) <- c("richness", "pielou", "connectance", "weighted_mean_trophic_lvl", "synchrony", "cv_sp", "stability") 
colin_stab_sem <- map(mod_list, car::vif)

tmp <- map(colin_stab_sem, enframe)
  
# colinearity table
colin_sem <- tibble(vif = tmp, mod = names(mod_list)) %>%
  unnest(vif) %>%
  spread(name, value)
...
plot_residuals <- function (x, y = "Diagnostic") {
  plot.lme(x)
  #title(main = y, xlab = "Fitted values", y = "Residuals")
}
plot.lme(mod_list[[7]])
p_resid_stab_sem <- map2(mod_list, names(mod_list) , plot_residuals)
