...
functioning has been a central research topic in ecology [@may_will_1972,
@hector_plant_1999, @loreau_biodiversity_2001, @hooper_effects_2005,
@duffy_why_2009, @isbell_forest_quantifying_2018]. Current biodiversity losses
...
Species richness has been reported to increase productivity in grasslands
[@grace_integrative_2016], but complex links between between species richness,
environment, productivity and total biomass showed that species richness
decrease total biomass [@grace_integrative_2016] contrary to previous
assessment [@hector_plant_1999, @tilman_biodiversity_2014] because of
competition for light between plants [@grace_integrative_2016]. Complementarity
between species in resource exploitation [@cardinale_species_2002] leading to
overyielding (CHECK) has been proposed to explain positive relationship between
productivity and species richness, while species richness effect can also rise
from the statistical probability to end up with a highly productive species
...
of abundance or biomass through time). @may_will_1972 stimulated a lot of
debate and fecund research, showing that species richness, probability of
interacting and average interaction strength destabilizes communities in which
species interact randomly. Empirically, consistent works showed that species
richness increased community stability in grasslands
...
components can explain variation of community stability, (1) individual species
stability and (2) the species synchrony [@loreau_species_2008,
@thibaut_understanding_2013]. Positive [@tilman_biodiversity_2006], neutral
...
species stability and species richness have been found. On the other side,
studies consistently found that species richness decreased synchrony between
species [@loreau_species_2008, @thibaut_understanding_2013,
@bluthgen_land_2016; @olivier_independent_nodate]. However, as far as we know,
less studies have reported empirical assessment of stability-richness
relationship in trophic network [but see @thebault_stability_2010].

Conversely, an extensive literature exists on the effects of network structure
on stability in mutualistic and trophic networks [@angelis_stability_1975, @stouffer_,
@thebault_stability_2010, @barbier_pyramids_2019,
@shanafelt_david_w._stability_nodate, add ref]. Particularly, in trophic
networks, most studies are theoretical and consider asymptotic stability to
small perturbation. Studies showed that compartimentalization [@stouffer_] can
increase persistence, while nestedness can either increase [@bascompte,
@thebault_stability_2010] or decrease persistence [@thebault_stability_2010].
Connectance can decrease [@thebault_stability_2010] or increase
[@angelis_stability_1975] resilience of trophic network depending on the
functioning of networks [@angelis_stability_1975].
...
Trophic network are assembled both vertically and horizontally. Studies showed
that the vertical position in the trophic network as well as the strength of
top-down and bottom-up regulation determine biomass and stability at
equilibrium  [@shanafelt_david_w._stability_nodate, @barbier_pyramids_2019].
However studies on stability-diversity have focused on either vertical
diversity or horizontal but rarely both. @zhao_horizontal_2019 have shown that
horizontal and vertical diversity respectively increase and decrease stability
...
A large inconsistency remains between theoretical and empirical assessment of
stability-diversity in trophic communities. Theoretical studies have
ignored temporal variability and the relative contribution of synchrony and
species variability while empirical studies about temporal variability in natural 
communities have ignored trophic communities and the effect of trophic structure. 
...
Our goal was to assess stability-diversity in natural trophic community with the
effect of the trophic structure. We combined together the effect of vertical
and horizontal diversity and thus aimed to bridge the gap between theoretical
and empirical studies. We used an standardized monitoring of freshwater fish
...
interaction networks corresponding to each sampling event, based on size
relationships and knowledge about their ontogenic diet shift. 
...
We investigated the effects of the community structure on total biomass and on
synchrony and species variability, which in turn determine community
variability. We have also included the effect of environmental drivers which
can directly affect community variability or indirectly via community structure.
Finally, we assessed the effect of horizontal diversity on trophic group
biomass and variability. 
...
little and big streams (detailed in Supplementary methods).  The sampling was
performed by electric fishing. All the stunned fish species were identified and
the individuals counted. Fish were classified in batch according to species and
size class. Depending on the size class, individual fishes were measured or
only a subsample. The size of individual fishes in subsampled batches were
inferred under normality distribution assumption (detailed in Supplementary
material).
...
relationships as in many studies [@brose_predator_2019, @poisot_structure_2016,
...
for which we had no biomass) ($\overline{T} = \sum_i^N{B_i * T_i} \times
...
The body mass of each fish was approximated from their body size with an
allometric law ($B = 0.01 \times (l^3.03)$, $B$ being the body mass in grams and $l$ the
body size in millimeters). We have worked with body mass instead of abundances to avoid to sum
abundances of fishes that had really different sizes and so body masses. Body
mass are more meaningful in terms of ecosystem description. The biomass of each
trophic species (and species) was divided by the surface sampled (measured in
square meters) during the sampling event. The total biomass in a given
community are so expressed in $gm^{-2}$.
...
stab_troph_mod <- lmer(data = stab_div_troph,
  formula = log_stab_std ~ log_rich_tot_std + troph_group + log_rich_tot_std:troph_group + (1 | basin)
  #control = ctrl
)
stab_troph_boot <- bootMer(stab_troph_mod, mySumm2, nsim = 200)
...
```{r}
richness_lab <- "Total richness (m2^-1) (Log 10)"
stab_lab <- "Biomass stability (Log 10)"
bm_lab <- "Total biomass (g/m2) (Log 10)"
...
p_stab_div_troph <- stab_div_troph %>%
  ggplot(aes(x = log_rich_tot_std, y = log_stab_std, color = troph_group)) +
  geom_point() +
...
p_bm_div_troph <- stab_div_troph %>%
  ggplot(aes(x = log_rich_tot_std, y = log_bm_std, color = troph_group)) +
  geom_point() +
  ylim(lim_log_bm) +
  xlim(lim_log_rich_tot) +
  ggplot2::scale_color_manual(
