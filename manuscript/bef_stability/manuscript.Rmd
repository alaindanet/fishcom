...
S_i &= 10^{\beta_0} \times 10^{\beta_R\log_{10}x_{Ri}} \times
10^{\epsilon_{ij}}\\
...
rich_on_bm <- get_rich_on_bm(fit = bm_all_params,
  p_val_thl = 0.05)
rich_on_sync <- get_rich_on_bm_comp(fit = bm_all_params,
  p_val_thl = 0.05,
  bm_comp = "log_sync")
rich_on_cv_sp <- get_rich_on_bm_comp(fit = bm_all_params,
  p_val_thl = 0.05,
  bm_comp = "log_cv_sp")
```
...
group but that high trophic was responsible for the negative stability-diversity
relationship.
...
The total effect of river size on biomass stability was as negative as species richness,
indicating that big rivers had a high negative effect on stability.
...
increasing species richness [@winemiller_must_1989, @thebault_stability_2010],
because a small fraction of links are realized because of food diet constraints.
In theoretical studies, connectance can be tuned from 0 (no links) to 1 (all
possible links are present) [@angelis_stability_1975]. SAMEFORTLVL
