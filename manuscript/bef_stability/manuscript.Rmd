...
stab_div_troph <- temporal_network_metrics %>%
  ...
  dplyr::select(station, troph_group, biomass_stab, richness_med)
```
...
#stab_div_colin <- car::vif(stab_div_mod)
...
label_rsq <- "paste(
  \"Conditional italic(R) ^ 2 == \", round(stab_div_sum$Conditional, 2),
  \", Marginal italic(R) ^ 2 == \", round(stab_div_sum$Marginal, 2))"
# Plot left  
p_stab_div <- sem_data %>%
  ...
  = "grey20", alpha = .5) +
  annotate("text", x = 1.0, y = .8, label = paste(
  " Marg. R^2 = ", round(stab_div_sum$Marginal, 2),
  ", Cond. R^2 = ", round(stab_div_sum$Conditional, 2)
  )
    )

# Right plot
p_stab_div_troph <- stab_div_troph %>%
  ggplot(aes(x = richness_med, y = biomass_stab, color = troph_group)) +
  geom_point() +
