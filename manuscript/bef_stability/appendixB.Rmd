...
then one to three electric fishing events are performed. We selected only the
data from the first event to homogenize the sampling effort. In deeper and wider
streams (depth >= 0.7m), this protocol is partial and is carried out along the
banks and the channel and it is performed by boat and by foot (along the bank).
We selected only the sampling done over the bank because the
channel was not systematically sampled. Two types of partial protocols take
place: "partial over bank" and "partial by points", which are respectively
a continuous and discrete fishing events. We considered both types of partial protocols
...
Heterogeneity in the fishing date may introduce variability due to different
...
where the median standard deviation of the sampling month was 0.7 month, i.e. three
...
problem in the batch. Finally, in the dataset used in the analysis, 
`r na_length` individuals had missing length over `r nrow_fish_length`
individuals, and body size was inferred for these individuals.
...
Table S2). The diet information described two to three life stages by
...
trophic_building_caption <- paste0( "Definition of trophic species and trophic
  interactions. (1) Each
...
feeding interactions (1) among trophic species, (2) among trophic species
and resource nodes and (3) among resource nodes.

#### Local food-webs 

The local food-webs (i.e. networks at each sampling event) were obtained by
determining the trophic species of each fish individual in the community.
According to the trophic species present in a given community, we set the
trophic interactions between them as defined in the metaweb.  All the resource
nodes were assumed to be present in each local network.  Thus, the structure of
the local food-webs varied according to the presence of the trophic species.
...
Oxygen Demand (BOD) variables were obtained. In the following section, we 
present how we averaged each variable to match the annual fish monitoring
events. We also detail how we interpolated these variables to match the
...
flow and BOD at the sampling sites. Indeed, stream characteristics make interpolation more
...
country. We had to find a trade-off between the number of data (to keep 
...
which had the smallest standard deviation.
...
increases, decreases, no trends or and a mixture of the above. Theoretically, however, the CV
...
is generic enough to encompass a diversity of temporal trajectories. The
classification necessitates three steps. The time series are first centred such
as the mean value is equal to 0.
Then, the dimensionality of the time series is reduced by a Piecewise
...
time series typically follow a normal distribution ($N(0,1)$), a breakpoint can
...
the highest ($a$, $b$, $c$, ...). We set two breakpoints, thereby simplifying
our time series into three periods.
...
\begin{table}
\caption{\label{tab:hyp-sem} Presentation of the hypothesis underlying the
effects included in the Structural Equation Models with supporting references.}
\begin{center}
\begin{tabular}{ m{7cm}| m{1cm}| m{6cm}}
    \hline
    Link & Sign & References\\
    \hline
...
caption_mod_res <- paste0(
  "Residuals versus fitted values from the linear models included in the structural
  equation models."
) 
```
...
```{r sensitivity, cache = TRUE, message = FALSE, fig.dim = c(10.5, 7), fig.cap = "Sensivity analysis with different dataset"}
library(furrr)
...
```{r}
sync_cap <- paste0(
  "Synchrony between trophic species pairs that are trophically linked or not.
  There is one point by trophic species pair and by site. The figure shows that
  the trophic species pairs that are trophically linked are more synchronous
  than the ones that are not trophically linked. Please see the definition of
  synchrony in the methods of the main text.") 
```

```{r, fig.cap=sync_cap}
knitr::include_graphics(mypath("manuscript", "bef_stability", "result", "synchrony_species_pairs.png"))
```
