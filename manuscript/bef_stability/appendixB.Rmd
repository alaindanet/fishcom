---
title: Appendix B of Species richness and network structure jointly drive total biomass and its temporal stability in fish communities
...
is used. It consists in placing anodes across the width of the
stream. The fishermen made from one to three row
...
The former is continuous while the latter is discrete. Furthermore, in the "partial
by point" protocol, the channel can be sampled as well. We kept only the "partial
by point" sampling in which all the sampling points were done over the bank to
homogenize the sampling. Then, we merged partial over bank and partial by 
point protocols. Some sampled stations had mixed characteristics in which mix of
partial and complete protocols were applied depending of the depth of the stream 
at the time of sampling. In this case, we kept only the protocol that was the
most used. 
...
was two in a given year, the one with the most species and then individual
...
to one (0.7) within stations. Additionally, we removed sampling events, in a
given station, in which the sampled length along the stream was outside the median
...
During the electric fishing, and before that fish body size are measured, the
fishes are regrouped by batches. Each batch contains one species but there can
be several batches by species. Body size assessment are different according to
the batches. There are four types of batches: N, I, S/L and G. The batches N and
I contains the biggest individuals which are normally less numerous. Then
the body size of all the individuals in the N and I batches is recorded. The batches S/L
contains fishes of intermediate sizes. Inside each S/L batches, the operator
tries to make batches as homogeneous as possible in terms of body size. Then, the body
...
homogeneous as possible in a given batches. Then body size of the shortest and the
longest individuals are recorded.
...
because the body size of all individuals was recorded. The body size ($l$) of the
fishes in the S/L and G was inferred under the assumption that its distribution
...
removed batches in which the total number of individuals was not recorded. In lot
"G", we removed batches where $l_{min} \geq l_{max}$ or if at least one of the
...
In the S/L lot, we removed batches in which the number of individuals in the
...
metaweb_caption <- paste0(
  "The metaweb which describes the interarctions between all the 412 trophic species, each is
...
```{r metaweb, fig.cap=metaweb_caption}
...
size class ends up in a life stage containing piscivorous diet, i.e. if there was
an overlap between them. We were more liberal for piscivory determination to
avoid 
missing potential fish-fish trophic interactions which were the main interactions
...
recording station, we applied a moving window averaging to get rid of seasonal
effects. For temperature, the moving was performed on daily average temperature.
Then we took the annual average of each year and each variable (i.e. one value
by year and by station).
...
methods to interpolate water temperature, flow and BOD.
...
up with four region of interpolation: West, East, South and North.
...
The CV of biomass over time can cover multiple dynamics, including increases,
...
pca_caption <- paste0(
  ...
  "(A) Axis 1 was positively related to the stream size, axis 2 was related to
  the stream slope and altitude and negatively related to average water temperature and
  BOD. (B) Eigenvalues distribution, we kept the first two axis for the data analysis. ",
  "Please note that we took the inverse of the second axis in the data analysis,
  to make the axis more easily interpretable, i.e. positively related de average
  water temperature."
...
\caption{\label{tab:hyp-sem} Presentation of the hypothesis supporting the links
of the Structural Equation Models with the references supporting the links.}

\begin{center}
\input(~/Documents/post-these/mnhn/fishcom/manuscript/bef_stability/table/hypothesis_sem.tex)
\end{center}
\end{table}
```
