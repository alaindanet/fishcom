---
title: Appendix B of Species richness and food-web structure jointly drive community biomass and its temporal stability in fish assemblages 
...
nocite: |
    ...
    @petchey_predicting_2010| @winemiller_must_1989
...
The monitoring of the fish communities occurs in a variety of stream size, shape
and depth. The stream characteristics constrain the type of protocol that can be
applied. Small streams (depth < 0.7m across the entire width) are sampled with
the complete protocol. The protocol consists of placing anodes across the entire
width of the stream. The fishermen made from one to three rows of electric
fishing. We selected only the data from the first row to homogenize the sampling
effort. In deeper and wider streams (depth >= 0.7m), the sampling was partial
over the bank and the channel and performed by boat and by foot (along the
bank). We selected only the sampling done over the bank because the channel was
not systematically sampled. Two types of partial protocols takes place: partial
over bank and partial by points, which are respectively continuous and discrete.
Furthermore, in the "partial by point" protocol, the channel was sometimes
sampled as well. We selected only the "partial by point" sampling in which all
the sampling points were done over the bank to homogenize the sampling. Then, we
merged partial over bank and partial by point protocols. Some sampled stations
were located in stream with intermediate characteristics where both protocols
were alternatively used depending on the state of the stream at the time of
sampling. For these sites, we only kept the samples that had been carried out
with the most used protocol.

Heterogeneity in the sampling date may introduce variability that is not related
to ecological processes such as the presence of juveniles. In order to reduce
heterogeneity in the sampling date, only one sampling per year was retained if
there was more in a given year, the one with that had the most species and then
the most individuals. Finally, most of the fish sampling occurred in September
(Median: mid-August) and the median standard deviation of the sampling month was
inferior to one (0.7) within stations, i.e. 3 weeks. To further reduce sampling
heterogeneity, we only kept the sampling in which the length of stream sampled
along the stream was inside the median length sampled more or less 30%.

### Batches definition and body size measurement 

Once the fishes have been caught, they are divided into different batches (Fig.
\@ref(fig:p-body-size)). Each batch contains only one species but there may be
several batches for one species. The body size of the fishes is measured
differently for each type of batch.  There are four types of batches: N, I, S/L
and G. The batches N and I contain the biggest individuals. In the N and I
batches, the body size of all the individuals is recorded. The batches S/L
contain fishes of intermediate sizes.  Inside each S/L batch, the operator tries
to make batches as homogeneous as possible in terms of body size. Then, the body
size of a subsample of at least 20 individuals is recorded. The G batches
contain individuals of small body size, with care that body size is as
homogeneous as possible in a given batches.  Then body size of the shortest and
the longest individuals are recorded.
...
body size distribution follows a normal law ($l ~ N(\mu, \sigma)$).  For an S/L
batch, the mean $\mu$ and the standard deviation $\sigma$ were simply the
average and the standard deviation of the body size of the subsample recorded.
For a G batch, the average was approximated by $\hat{\mu} = (l_{min} + l_{max})
2^{-1}$ and the standard deviation by $\hat{\sigma} = (l_{min} + l_{max})
4^{-1}$. With the estimations of the mean and the standard deviation, the number
...
[$l_{min}$; $l_{max}$] for the G batches. Body size distribution were generated
using the R function `truncdist::rtrunc`.

Batches that did not meet certain conditions were excluded. First, we also
excluded batches for which the total number of individuals was not recorded. We
also excluded G batches for which $l_{min} \geq l_{max}$ or if
at least one of the measurement was missing. Additionally, a given G batch was
excluded when the number of individuals fell below five, as G batches normally
contained more individuals. We finally excluded S/L batches for which the
number of measured individuals was inferior to 10. Finally, in the dataset used
in the analysis, `r na_length` individuals had missing length over 
...
Some species were identified based on their morphotype, but they
belong to the same species based on fecundation criteria. We merged those
morphotype species into one species. There was
`r length(unique(names(sp_to_replace())))` species to merge which belonged to 
...
only seven had a piscivorous life stage and only three had a strict piscivorous
life stage. Fish species were then largely omnivorous.
...
trophic_building_caption <- paste0( "Building of the metaweb. (1) Each
  species are divided in nine body size classes. We gathered information about
  the food diet of each fish species and how food diet varies according to
  ontogeny. (2) The life stage of a trophic species was determined with the
  median of the body size class. A trophic link was set between the trophic
  species and the resources associated to this life stage. (3) For a piscivorous
  trophic species, the trophic links between fishes were defined according to
  the predators/prey body size ratio. The predation window of the trophic
  species ranged from 3% to 45% of the median of the body size range. A trophic
  link between the predator trophic species and every trophic species whose
  midpoint of bodysize range was included in the predation window.") 
...
metaweb_caption <- paste0(
  "The metaweb describes the trophic interactions between all the 412 trophic species, each is
  represented by a node and each color represents a species (S = 45). The seven
  resource nodes (7) are drawn in grey. The species names corresponding to the
  three digit codes are found in Table S1."
)
```

```{r metaweb, cache = FALSE, fig.cap=metaweb_caption}
knitr::include_graphics(mypath("manuscript", "bef_stability", "figs", "metaweb.png"))
```
...
the diet and the body size of the trophic species. `r length(removed_sp_metaweb)` 
species were removed because information about their diet was missing. It
represented  `r nrow(removed_ind_metaweb)` individuals over
`r nrow(length_sem_analysis)` individuals.

The life stage to which a trophic species belongs was determined by the median of
the body size range of the trophic species, each life stage being delimited by a body
size range. In turn, this trophic species has trophic interactions with the
resource nodes of its life stage. The trophic interactions between a
predator and a prey were determined by predator-prey body size ratio
[@brose_predator_2019; @poisot_structure_2016; @schneider_body_2012]. For a
given trophic species, we first determined if its body size range overlap
with a piscivorous life stage (Fig. \@ref(fig:p-net-inf)). We were then more
liberal for piscivory determination than for fish-resource interactions, to
avoid missing potential interactions which were the main interactions in our
community. If the trophic species was piscivorous, we determined its prey size
range from 3% to 45% of the median of its size range. A trophic interaction was
set for each trophic species whose median of its size class fell into the prey
range of the predator trophic species. To sum up, the metaweb contains feeding
...
The local trophic networks (i.e. one network by
...
community, we set the trophic interactions between them according to the metaweb. 
...
The procedure to obtain altitude, slope, width and depth of the streams are fully
...
Oxygen Demand (BOD) variables were obtained. The goal was to obtain an annual
mean for each variable, to match the yearly resolution of the fish monitoring.
Furthermore, those variables are measured at different locations than the fish
monitoring. 
...
an
hourly, daily and monthly frequency. Water temperature and flow are recorded by
automated sensors while BOD is collected manually because its measurement
requires laboratory analysis. BOD data collection is coordinated by the Agence
de l'eau which realized the analysis itself or order it to a company.  

```{r, cache = FALSE}
pressure_cap <- paste0(
  "Map of the environmental variables obtained for the 99 sites used in the
  analysis. Slope is expressed in degree, altitude in meter, distance from
  ...
  mg of dissolved oxygen, water flow is expressed in cube meter per second and
...
To get rid of seasonal effects of temperature, we applied a moving window
averaging to each time series. The moving window was applied on daily average
temperature. Then we took the annual mean of each year and each variable
(i.e. one value by year and by station).
...
based on euclidean distance while euclidean distance is complicated to consider
in streams because it had to follow the path of the streams. Furthermore, two
streams can be close by but not connected and thus the value of their variables might 
not be related or only a few. For those reason, we used stream spatial network
methods to interpolate water temperature, water flow and BOD.
...
France. We had to find a trade-off between the number of data (to keep enough
high to perform the interpolation) and the level of dataset splitting. We then
merged some hydrographic basin to end up with four regions of interpolation:
West, East, South and North.

Then we modelled each variable as dependant only on the covariation between
sites and taking in account stream network. To model the autocovariance between
sites, we used the linear-sill and mariah functions for taildown and tailup
...
cv_dbo_temp_flow %>%
  ...
  kable(
    ...
    caption = paste0("Results of the cross-validation for the interpolation of
      BOD, temperature and flow. Average over 4 interpolations are shown. The
...
[@kefi_advancing_2019]. In order to approach the condition of equilibrium, we kept only the
stations that have met this criteria. To characterise the biomass temporal
dynamics, we used the symbolic aggregated approximation method (SAX), which is
commonly used in time series analysis and implemented in the `jmotif` R package
[@senin_grammarviz_2018].  This method classifies time series rather than
quantifies trends and so is generic enough to encompass a diversity of temporal
trajectories. The classification necessitates three steps. First, the time
...
by a Piecewise Aggregation Approximation (PAA) method. Each time series is
divided in $N$ frames of equal size that allows to reduce the dimensionality
of the time series. For each frame, we simply kept the mean value of the time
series. The last step is to qualify each frame into string. Because values of
normalized time series typically follows a normal distribution ($N(0,1)$), a
breakpoint can be computed by dividing the area under the normal distribution
in equal parts. The algorithm attributes an alphabetic letter to each part from
...
respectively monotonic increasing and decreasing trajectories. For data
analysis, we kept only the $bbb$ which represent time series that showed no
temporal tendencies, with respect to our methodology.
...
\begin{tabular}{ m{7cm}| m{1cm}| m{6cm}}
    ...
    Link & sign & reference\\
    \hline
    PCA1 Avg stream size $\to$ Species richness & (+) & Post et al. (2000); Doi et al. (2009); McHugh et al. (2010) \\
    \hline
    PCA2 Avg temperature $\to$ Species richness & (+/-) & O’Gorman et al. (2017); Gauzens et al. (2020) \\
    \hline
    PCA1 Avg stream size $\to$ Connectance & (?) & \\
    \hline
    PCA2 Avg temperature $\to$ Connectance & (+/-) & Petchey et al. (2010) \\
    \hline
    ...
    \hline
    Connectance $\to$ Synchrony & (+) & Teng and McCann (2004)\\
    \hline
    Avg trophic level $\to$ Synchrony & (+) & Raimondo et al. (2004)\\
    \hline
    PCA1 Avg stream size $\to$ Synchrony & (?) & \\
    \hline
    PCA2 Avg temperature $\to$ Synchrony & (+) & Loreau and de Mazancourt (2008)\\
    \hline
    Species richness $\to$ CVsp & (+) & Tilman (1996) \\
    \hline
    ...
    \hline
    PCA2 Avg temperature $\to$ CVsp & (+) & O’Gorman et al. (2017); Tabi et al. (2019)\\
    \hline
    ...
    \hline
    Connectance $\to$ Total biomass & NA & Wang and Brose (2018)\\
    \hline
    ...
    \hline
    PCA2 Avg temperature $\to$ Total biomass & (+/-) & O’Gorman et al. (2017)\\
    \hline
    \end{tabular}
\end{center}
\end{table}
...
hist_var_caption <- paste0(
  "Distribution of the variables included Structural Equation Models The
...
format_coef_table(stab_sem_rich) %>%
  mutate(Std.Estimate = cell_spec(Std.Estimate, bold = ifelse(P.Value <= 0.05, T, F), format = "latex", escape = F)) %>%
  kable(
    format = "latex",
    booktabs = T,
