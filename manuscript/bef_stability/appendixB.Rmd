...
the "complete" protocol. This protocol consists of placing anodes across the
entire width of the stream. As the fishermen made from one to three rows of
electric fishing, we selected only the data from the first row to homogenize the
sampling effort. In deeper and wider streams (depth >= 0.7m), the sampling is
...
samples that had been carried out with the most used protocol.

Heterogeneity in the sampling date may introduce variability in the fish
communities such as the presence of juveniles. In order to reduce heterogeneity
in the sampling date, only one sampling per year was retained if there were more
in a given year, the one that had the most species and then the most
individuals. Finally, most of the fish sampling occurred in September (Median:
mid-August) and the median standard deviation of the sampling month was inferior
to one (0.7) within stations, i.e. 3 weeks. To further reduce sampling
...
also excluded G batches for which $l_{min} \geq l_{max}$ or if at least one of
the measurement was missing. Additionally, a given G batch was excluded when the
number of individuals felt below five, as G batches normally contained more
individuals. We finally excluded S/L batches for which the number of measured
individuals was inferior to 10, because it indicates a potential problem in the
batch. Finally, in the dataset used in the analysis, `r na_length` individuals
had missing length over `r nrow_fish_length` individuals, and body size was
inferred for these individuals.
...
subspecies into one species. There was
...
Diet information was obtained from fishbase.org and literature (Appendix A, Table S2). The
...
trophic_building_caption <- paste0( "Building of the metaweb. (1) Each
  species is divided into nine body size classes. We gathered information about
...
metaweb_caption <- paste0(
  "The metaweb describes the trophic interactions between all the 412 trophic species. Each trophic species is
...
by a body size range. In turn, this trophic species has trophic interactions
...
whose corresponding size class median felt into the prey range of the predator trophic
species (Fig. \@ref(fig:p-net-inf)). To sum up, the metaweb contains feeding interactions (1) between
trophic species, (2) between trophic species and resource nodes and (3) between
resource nodes.
...
fishing protocol, they were easier to manage in the analysis. Then, we detail
...
Furthermore, the available data for these variables were measured at different
locations than the fish monitoring and then necessitates a step of spatial
interpolation.
...
sites taking in account the stream network. To model the autocovariance between
...
This method classifies time series rather than quantifies trends and so is
generic enough to encompass a diversity of temporal trajectories. The
classification necessitates three steps. The time series are first normalized. Then, we reduced the dimensionality of the time series by a Piecewise
...
last step is to qualify each frame into string. Because values of normalized
...
\begin{tabular}{ m{7cm}| m{1cm}| m{6cm}}
    ...
    \hline
    Connectance $\to$ Total biomass & (+/-) & Thébault & Loreau (2003); Poisot
    et al. (2013)\\
    \hline
