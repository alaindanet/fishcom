...
### class connectance versus species connectance

```{r}
cl_ct <- temporal_network_metrics_classes %>%
  select(station, connectance_med, nbnode_med) %>%
  rename(cl_ct_med = connectance_med, cl_node_med = nbnode_med)
myload(temporal_network_metrics, dir = mypath("data", "species"))
sp_ct <- temporal_network_metrics %>%
  select(station, connectance_med, nbnode_med) %>%
  rename(sp_ct_med = connectance_med, sp_node_med = nbnode_med)
...
sp_net <- sp_network_metrics %>%
  select(opcod, connectance, nbnode) %>%
  rename(sp_ct = connectance, sp_node = nbnode)
cl_net <- cl_network_metrics %>%
  select(opcod, connectance, nbnode) %>%
  rename(cl_ct = connectance, cl_node = nbnode)

net_comp <- sp_net %>% 
  left_join(cl_net, by = "opcod")
```
...
net_comp %>%
  ...
  ggplot(aes(x = ct, fill = type)) +
  geom_histogram(position = "identity", alpha = .2) +
  xlab("Connectance")
```


### Weighted average trophic level: class vs species network

```{r}
sp_net <- sp_network_metrics %>%
  select(opcod, w_trph_lvl_avg, nbnode) %>%
  rename(cl_tlvl = w_trph_lvl_avg, cl_node = nbnode)
cl_net <- cl_network_metrics %>%
  select(opcod, w_trph_lvl_avg, nbnode) %>%
  rename(sp_tlvl = w_trph_lvl_avg, sp_node = nbnode)
