...
# Compute SEMs
sem <- compute_stab_sem_rich_beta(.data = sem_data,
...
#library(future)
#plan(multiprocess)
sem_sensivity_data <- list(full = op_analysis, wo_holes = op_analysis_wo_holes, bbb = op_analysis_bbb)
sem_l_data <- furrr::future_map(sem_sensivity_data,
...
```{r, fig.dim = c(10, 5)}
p_sensitivity_stab_sem <- map(sem_l_data, ggpiecewisesem)
