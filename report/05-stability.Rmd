...
plot_grid(p_bbb_stab_div, p_bbb_map, ncol = 2)
```
...
```{r bimod-stab-div, fig.cap = "Relationship between stability and diversity by spliting dataset in two according to an environmental variable. The threshold for the dataset split is the median of the environmental variable."}
...
env <- c("width_river_mean", "avg_depth_station_mean", "lat", "long",
  "temperature", "flow", "d_source", "DBO")
mult_plot_stab_div_bin <- map(env, ~plot_bin_stab_div(env = .x))
plot_bin_stab_div(env = "width_river_mean")

plot_grid(plotlist = mult_plot_stab_div_bin)
```
...
```{r}
sync_com_troph_without_ang <- network_analysis %>%
...
```{r}
sync_com_troph <- network_analysis %>%
  select(opcod, composition) %>%
  unnest(composition) %>%
  mutate(species = str_extract(sp_class, "[A-Z]{1,3}")) %>%
  select(-sp_class) %>%
  ...
  rename(bm_st = biomass_avg, div_st = richness_avg)
```
...
```{r, fig.dim = c(7, 14)}
sync_com_troph %>%
  select(troph_group:cv_com, div_st) %>%
  gather(comp, stab, synchrony:cv_com) %>%
  ggplot(aes(x = div_st, y = stab)) +
  geom_smooth(method = "loess", se = FALSE) +
  labs(x = "Trophic species diversity", y = "Stability component") +
  geom_point() +
...
The figure \@ref(fig:cbt-top), left panel) shows that for the trophic group 2, LOF, GOU and
VAI were the species that contributes the most to the the CV of the community
and to its components (CVsp and synchrony). For the 3 trophic group (Figure
\@ref(fig:cbt-top), right panel), the ANG and CHE contribute the most to
stability and its facets. Interestingly, the contribution of the species to
the community CV were quite asymetric toward positive values, meaning that
species tend to stabilize more than destabilize communities. 

```{r var-cbt, fig.dim = c(10, 7)}
var_cbt <- sync_com_troph %>%
  unnest(contrib) %>%
  ...
  left_join(select(cv_bm_div_troph_grp, station, troph_group, div_st),
...
pielou <- network_analysis %>%
  ...
  summarise(
    richness = n(),
    shannon = diversity(biomass, index = "shannon"),
...
pielou %>%
  ...
  ggplot(aes(x = richness_avg, y = pielou_med)) +
  geom_point() +
