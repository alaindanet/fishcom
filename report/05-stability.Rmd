...
%>%
  select(station, troph_group, biomass_cv, richness_avg, nbnode_med) %>%
  gather(richness_type, div, richness_avg, nbnode_med) %>%
  ggplot(aes(x = div, y = biomass_cv, color = troph_group)) +
  facet_grid(cols = vars(richness_type), scales = "free_x") +
  geom_point() +
  stat_poly_eq(
    formula = y ~ x,
