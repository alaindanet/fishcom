...
lav_data <- compute_sem_dataset(
  com = temporal_community_metrics,
  network = temporal_network_metrics, 
...
```{r sensitivity, fig.dim = c(14, 7), fig.cap = "Sensivity analysis with different dataset"}
myload(op_analysis, op_analysis_wo_holes, op_analysis_ov, dir = mypath("data"))
sem_l_data <- map(list(full = op_analysis, wo_holes = op_analysis_wo_holes, ov = op_analysis_ov),
  ~build_dataset_get_sem_coefficient(
    .op = .x,
    dest_dir = mypath("data", "species"))
)

sem_results <- map(sem_l_data,
...
prod_sem <- compute_prod_sem(.data = sem_data)
...
car::vif(lme(prod ~ log_rich + piel + c_c + t_lvl + beta_bin_c + evt_med1 + evt_med2 + evt_cv1 + evt_cv2, random = ~ 1 | basin, data = sem_data)) %>%
  enframe %>%
  kable
```
