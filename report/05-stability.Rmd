...
sync_com_troph %>%
  filter(troph_group != 1) %>%
  select(station, troph_group, cv_com, troph_sp_nb_med) %>%
  ggplot(aes(x = troph_sp_nb_med, y = 1/cv_com, color = troph_group)) +
  geom_point() +
  stat_poly_eq(
    formula = y ~ x,
    eq.with.lhs = "italic(hat(y))~`=`~",
...
sync_com_troph %>%
  ...
  summarise(
    sum_cbt_cv_com = sum(abs(cbt_cv_com)),
    sum_cbt_cv_sp = sum(abs(cbt_cv_sp)),
    sum_cbt_sync = sum(abs(cbt_sync))
    ) %>%
  arrange(desc(sum_cbt_cv_com)) %>%
  slice(1:10) %>%
  gather(comp, contrib, sum_cbt_cv_com:sum_cbt_sync) %>%
...
var_cbt <- sync_com_troph %>%
  ...
  var_cbt %<>%
	...
	ggplot(aes(y = contrib, x = div_st)) +
	  geom_point() +
	  scale_x_log10() +
	  facet_wrap(facets = ~comp, scales = "free_y") +
